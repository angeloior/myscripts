...
# count_entries ##################################################################################################################
def count_entries(dbcursor, checkname, table, attribute, warn_level, critical_level):
    #if (warn_level+critical_level)==0 :
    if dbcursor==None :
        print('3 optima-db-'+checkname+' '+attribute+'=0 CRITICAL, no connection')
        return
    try:
        dbcursor.execute("SELECT count(*) FROM "+table )
    except psycopg2.ProgrammingError as e:
        print('2 optima-db-'+checkname+' tables=0 CRITICAL malformed SQL-statement, '+table+' may not exist' )
        conn.rollback()
        return
    dbcount = cursor.fetchone()[0]
    #print(rows)
    if dbcount < critical_level:
        print('2 optima-db-'+checkname+' '+attribute+'='+str(dbcount)+' '+str(dbcount)+' '+attribute+' found in db, CRITCAL too less '+attribute)
    elif dbcount < warn_level:
        print('1 optima-db-'+checkname+' '+attribute+'='+str(dbcount)+' '+str(dbcount)+' '+attribute+' found in db, WARN too less '+attribute)
    else:
        print('0 optima-db-'+checkname+' '+attribute+'='+str(dbcount)+' '+str(dbcount)+' '+attribute+' found in db, OK enough '+attribute)


def get_deltatime_in_minutes(deltatime_object):
    result = (deltatime_object.days * 60 * 24) + (deltatime_object.seconds / 60)
    return int(result)
...
#check_link_flow ##################################################################################################################
def check_link_flow(dbcursor, warn_level, critical_level):
    if dbcursor==None :
        print("3 optima-qos-traveltime minutes=0 CRITICAL, no connection")
        return
    try:
